...
find_triples <- function(df, period = 360, num_vars = 2, type="eigen", ecdet="none", K=2, spec="longrun") {
    ...
    for(j in 1:length(stocks)) {
      for(k in 1:length(stocks)) {
        if((j>i) && (k>j) && (k>i)) {
          ...
          test_r2 = (johtest@teststat[1] < johtest@cval[1,3])
          test_r1 = (johtest@teststat[2] < johtest@cval[2,3])
